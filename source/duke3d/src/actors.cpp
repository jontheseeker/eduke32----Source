...
ACTOR_STATIC void G_MoveFX(void)
{
    ...
    while (spriteNum >= 0)
    {
        ...
        case MUSICANDSFX__STATIC:
        {
            ...
            {
                if (g_sounds[pSprite->lotag]->mode & SF_MSFX)
                {
                    ...
                    if (playerDist < spriteHitag && T1(spriteNum) == 0 && FX_VoiceAvailable(g_sounds[pSprite->lotag]->priority-1))
                    {
                        ...
                        char om = g_sounds[pSprite->lotag]->mode;
                        ...
                        g_sounds[pSprite->lotag]->mode |= SF_LOOP;
                        A_PlaySound(pSprite->lotag,spriteNum);
                        g_sounds[pSprite->lotag]->mode = om;
                ...
                if ((g_sounds[pSprite->lotag]->mode & (SF_GLOBAL|SF_DTAG)) == SF_GLOBAL)
                {
