...
int A_CallSound(int sectNum, int spriteNum)
{
    ...
    if (SFXsprite >= 0)
    {
        ...
        if (T1(SFXsprite) == 0)
        {
            if ((g_sounds[soundNum]->mode & (SF_GLOBAL|SF_DTAG)) != SF_GLOBAL)
            {
                if (soundNum)
                {
        ...
        else if (SHT(SFXsprite) <= g_highestSoundIdx)
        {
            ...
            if ((g_sounds[soundNum]->mode & SF_LOOP) || (SHT(SFXsprite) && SHT(SFXsprite) != soundNum))
...
int P_ActivateSwitch(int playerNum, int wallOrSprite, int switchType)
{
    ...
    switch (DYNAMICTILEMAP(basePicnum))
    {
        ...
        case REST_SWITCH_CASES:
        {
            ...
            {
                if (switchType == SWITCH_SPRITE && (g_sounds[hitag]->mode & SF_TALK) == 0)
